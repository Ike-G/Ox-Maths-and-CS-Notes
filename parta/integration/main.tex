    ...
    \block{Double Integrals}{
     ...
     \column{0.25}
      \block{$L^{p}$-spaces}{
        ...
        While these don't act as norms for $\mathcal{L}^{p} = \{f : \mathbb{R} \to \mathbb{R} : |f|^{p} \text{ integrable }\}$, with $\mathcal{N}$ the set of functions equal to $0$ almost everywhere, they act as norms on the quotient space $L^{p} := \mathcal{L}^{p} / \mathcal{N}$. \\

        \begin{theorem}[Minkowski's Inequality]
        \ For $p \ge 1$, $f, g \in \mathcal{L}^{p}$, $|| f + g||_{p} \le ||f||_{p} + ||g||_{p}$.
        \end{theorem}
        \hphantom{}
