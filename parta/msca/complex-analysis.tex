...
\block{Multifunctions}{
    ...
    With $\alpha = 1/k$ for $k \ge 2$, we can demonstrate that for $f : U \to V$ holomorphic, $f'(z_0) = 0$ implies that it is not injective. This is because $f(z) - f(z_0) = (z-z_0)^k g(z)$, and taking $|z - z_0| < \varepsilon$ so $|g(z) - g(z_0)| < |g(z_0)|$ we can then find a holomorphic function $\varphi : B(z_0, \varepsilon) \to V$ so $\varphi(z)^k \equiv \restr{g}{B(z_0, \varepsilon)}$, and thus $f(z) - f(z_0) = ((z-z_0)\varphi(z))^k$. Via the inverse function theorem applied to $z \mapsto (z-z_0)\varphi(z)$, it's then clear by the open mapping theorem that taking $w, w'$ two distinct $k$th roots of $\xi$, $f(z) - f(z_0) = \xi$ is solved by multiple $z$.
  }
\block{Integration}{
...
\block{Riemann Sphere}{
  ...
  Note that if $f : U \to V$ is biholomorphic (its inverse is holomorphic) then we must have that $f$ is conformal. \\
...
\block{Residue Theorem}{
  ...
  \begin{theorem}
  \ Let $f : U \to V$ be holomorphic on a neighbourhood of $z_0 \in U$, with $f'(z_0) \neq 0$. Then there exist neighbourhoods of $z_0$ and $f(z_0)$, $U' \subseteq U$ and $V' \subseteq V$ respectively such that $f : V' \to U'$ is holomorphic, and $f^{-1}(f(z_0)) = 1/f'(z_0)$.
  \end{theorem}
  \hphantom{}

  Write $f(z) - f(z_0) = (z-z_0)g(z)$ with $g(z_0) \neq 0$, and we want to show that for $w$ close to $f(z_0)$, $f(z) - w$ has exactly one zero for a small enough neighbourhood around $z_0$. To do this, with $\ell(z) := (z-z_0)g(z_0)$ and $h(z) := (z-z_0)g(z)$, we use Rouch\'{e}'s theorem on $\ell - (f(z) - w)$ and $h-\ell$ giving that if for $|z - z_0| = r$, $|h(z) - \ell(z)| < |\ell(z)|$, then $\ell$ and $h$ have the same number of zeros in $B(z_0,r)$. This is clear just via continuity, and as $\ell - $ has just one zero, thus $h(z) = f(z) - f(z_0)$ has one zero. Thus $(f(z) - f(z_0)) - (w - f(z_0))$ \textbf{This argument is weird ugh}
  }
\end{columns}
\end{document}
