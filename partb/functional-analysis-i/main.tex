...
\geometry{paperwidth=1600mm, paperheight=2600mm}
\makeatletter
...
\begin{columns}
  ...
  \block{Hilbert spaces}{
    ...
    \innerblock{Polarisation}{
      The constraints satisfied by an inner-product allow us to characterise Hilbert spaces significantly more than we're able to for Banach spaces. A critical tool for doing this is polarisation, allowing us to write norms in terms of their inner products. \\

      Note that for $x, z \in X$ a Hilbert space, via linearity and anti-linearity,
      \begin{align*}
        \Vert x + z \Vert^{2} &= \Vert x \Vert^{2} + 2 \Real{\langle x, z \rangle} + \Vert z \Vert^{2}.
      \end{align*}
      This immediately gives us an expression for $\langle x, z \rangle$ in terms of the space's norm if over $\mathbb{R}$, and we only need a bit more work to get the same for $\mathbb{C}$. With $z = e^{i\theta} y$ for $\theta \in \rightopen{0}{2\pi}$,
      \begin{align*}
        \Vert x + z \Vert^{2} &= \Vert x \Vert^{2} + 2 \Real(e^{-i\theta}\langle x, y \rangle) + \Vert y \Vert^{2},
      \end{align*}
      so with $\theta \in \{0,\pi/2\}$,
      \begin{align*}
        \langle x, y \rangle &= \frac{1}{2} \left(\Vert x + y \Vert^{2} + \Vert x + iy \Vert^{2}\right) - \Vert x \Vert^{2} - \Vert y \Vert^{2}.
      \end{align*}

      These methods are very similar to those also used above, but it's worth mentioning given that this is a very persistent idea.
    }
    \hphantom{}

    \begin{theorem}[Projection theorem]
      \ For any closed convex subset $Y$ of a Hilbert space $X$, for any point $x \in X$, there is a point $y^{*} \in Y$ such that
      \begin{align*}
        \Vert y^{*} - x \Vert &= \inf_{y \in Y} \Vert y - x \Vert.
      \end{align*}
    \end{theorem}
  ...
  \block{Bounded linear operators}{
    ...
    Note provided $T \in \mathcal{B}(X)$ is algebraically invertible, $T^{-1} \in \mathcal{B}(X)$ iff $\inf \Vert Tx \Vert / \Vert x \Vert > 0$. Indeed per the inverse function theorem in B4.2, if $X$ is a Banach space then $T$ is invertible iff is bijective.
  }
  \block{Finite-dimensional normed spaces}{
    With $(\mathbb{F}^{n},\Vert \cdot\Vert _{p})$ for some $p \in [1,\infty]$, $n \in \mathbb{N}$, $\mathbb{F} \in \{\mathbb{R}, \mathbb{C}\}$ we get the most basic Hilbert space. \\

    \begin{lemma}[H\"{o}lder's Inequality]
      \ For $1 \le p,q \le \infty$ such that $1/p+1/q = 1$, for any $\bm{x}, \bm{y} \in \mathbb{C}^{n}$,
      \begin{align*}
        \Vert \bm{x}\bm{y}\Vert _{1} \le \Vert \bm{x}\Vert _{p}\Vert \bm{y}\Vert _{q}
      \end{align*}
    \end{lemma}
    \hphantom{}
  ...
  \block{Theorem of Hahn Banach}{
    ...
    \begin{theorem}[Hahn-Banach (general sublinear version)]
      \ For $X$ a real vector space, $Y \subseteq X$ a subspace, $p : X \to \mathbb{R}$ sublinear, $f \in Y^{*}$, if $f \le \restr{p}{Y}$, then there is a linear extension $F \in X^{*}$ such that $F \le p$.
    \end{theorem}
    \hphantom{}
