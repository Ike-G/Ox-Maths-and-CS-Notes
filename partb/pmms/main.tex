...
\geometry{paperwidth=2550mm, paperheight=2500mm}
\makeatletter
...
\begin{columns}
  ...
  \block{Measurable sets and functions}{
    ...
    \begin{theorem}[Monotone Class Theorem]
      ...
      \begin{itemize}
              ...
              \item \ if $(f_{n}) \subseteq \mathcal{H}$, $f_{n} \to f$ monotonically increasing with $f$ bounded, then $f \in \mathcal{H}$,
      \end{itemize}
  ...
  \block{Measures on $\mathbb{R}$}{
    \begin{definition}[Measure space]
      \ A measure space is a triple $(\Omega, \mathcal{F}, \mu)$ such that $\Omega$ is a set, $\mathcal{F}$ is a $\sigma$-algebra on $\Omega$, and $\mu : \mathcal{F} \to [0,\infty]$ is countably additive ($\mu$ is then a measure on $(\Omega, \mathcal{F})$).
    \end{definition}
    \hphantom{}
  ...
  \column{0.1667}
  \block{Martingale Convergence}{
    \begin{definition}[Upcrossings]
      \ With a real sequence $(x_{n})$, $a, b \in \mathbb{R}$ with $a < b$, we can define the integer sequences $(\rho_{n})$, $(\tau_{n})$ such that
      \begin{align*}
        \rho_{n} &= \inf \{m \ge \tau_{n-1} : x_{m} \le a\} \\
        \tau_{n} &= \inf \{m \ge \rho_{n} : x_{m} \ge b\}.
      \end{align*}
      We can then define the sequence $(U_{n}([a,b],(x_{n})))$ as the number of upcrossings of $[a,b]$ by time $n$, $U([a,b],(x_{n}))$ the total number of upcrossings, by
      \begin{align*}
        U_{n}([a,b],(x_{n})) &= \sup \{k \ge 0 : \tau_{k} \le n\} \\
        U([a,b],(x_{n})) &= \sup \{k \ge 0 : \tau_{k} < \infty\}.
      \end{align*}
    \end{definition}
    \hphantom{}
    ...
    \innerblock{Backwards Martingales}{
      \begin{definition}[Backwards martingales]
      \ Let $(\mathcal{F}_{-n})$ with $\mathcal{F}_{-(n+1)} \subseteq \mathcal{F}_{-n} \subseteq \mathcal{F}$ for $n \ge 0$. A backwards martingale with respect to $(\mathcal{F}_{-n})$ is a sequence $(X_{-n})$ of integrable random variables such that $X_{-n}$ is $\mathcal{F}_{-n}$-measurable, and
      \begin{align*}
        \mathbb{E}\big[X_{-n+1} \,|\, \mathcal{F}_{-n}\big] = X_{-n}.
      \end{align*}
      \end{definition}
      \hphantom{}

      Since $X_{0}$ is integrable, we see that $(X_{-n})$ is automatically UI. We then need to adapt certain definitions such as with respect to upcrossings, but restore all of our normal results as expected, and in particular receive the following:

      \begin{theorem}
      \ Let $(X_{-n})$ be a backwards martingale with respect to $(\mathcal{F}_{-n})$. Then $X_{-n}$ converges a.s. and in $\mathcal{L}^{1}$ as $n \to \infty$ to $X_{-\infty} = \mathbb{E}\big[X_{0} \,|\, \mathcal{F}_{-\infty}\big]$.
      \end{theorem}
      \hphantom{}
      ...
      This theorem rests on defining $S_{n}/n$ as a backwards martingale with respect to the backwards filtration $\sigma(S_{-n},S_{-n+1},\dots) = \sigma(S_{-n},X_{-n+1},\dots)$, and using this to encode the independence of $S_{n}/n$ on the permutation of $\{X_{1},\dots,X_{n}\}$. We formalise this more generally as below: \\

      \begin{definition}[Exchangeability]
      \ The random variables $X_{1},\dots,X_{n}$ are exchangeable if $(X_{\varphi(1)},\dots,X_{\varphi(n)})$ has the same probability distribution for each permutation $\varphi \in S_{n}$ (with apologies for overloaded notation).
      \end{definition}
      \hphantom{}
